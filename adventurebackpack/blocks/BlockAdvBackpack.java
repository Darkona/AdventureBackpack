...
public class BlockAdvBackpack extends BlockContainer
{
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z)
	{
		...
		{
			l1 =
				((TileAdvBackpack) world.getBlockTileEntity(x, y, z)).luminosity;
		}
		return (l1 > l2) ? l1 : l2;
	}
	...
	{
		FMLNetworkHandler.openGui(player, AdventureBackpack.instance, 0, world,
			x, y, z);
		try
		{
			TileAdvBackpack te =
				(TileAdvBackpack) world.getBlockTileEntity(x, z, z);
	...
	{
		int dir =
			MathHelper.floor_double((player.rotationYaw * 4F) / 360F + 0.5D) & 3;
		createTileEntity(world, dir);
		if (stack != null && stack.stackTagCompound != null
			&& stack.stackTagCompound.hasKey("color"))
		{
			if (stack.stackTagCompound.getString("color").contains(
				"BlockRedstone"))
				dir = dir | 8;
	...
	public void registerIcons(IconRegister register)
	{
		...
		// BLOCKS
		blockIcon =
			register.registerIcon(ModInformation.ID.toLowerCase() + ":"
				+ Fluids.milk.getUnlocalizedName());
	...
	public boolean removeBlockByPlayer(World world, EntityPlayer player, int x,
		int y, int z)
	{
		TileAdvBackpack backpack =
			(TileAdvBackpack) world.getBlockTileEntity(x, y, z);

		if (!world.isRemote)
		{
			if ((player.isSneaking()) ? backpack.equip(world, player, x, y, z)
				: (player.capabilities.isCreativeMode) ? true : backpack.drop(
					world, x, y, z))
			{
			...
			for (int i = 0; i < inventory.getSizeInventory(); i++)
			{
				...
				if (stack != null)
				{
					...
					EntityItem droppedItem =
						new EntityItem(world, spawnX, spawnY, spawnZ, stack);
