...
public class BlockAdvBackpack extends BlockContainer{
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z) {
		if(world.getBlockTileEntity(x, y, z) instanceof TileAdvBackpack){
			return ((TileAdvBackpack)world.getBlockTileEntity(x, y, z)).luminosity;
		}
		return 0;//super.getLightValue(world, x, y, z);//((world.getBlockMetadata(x, y, z) & 4) >= 4) ? 15 : 0; 
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase player, ItemStack stack) {
		 ...
		 if(stack != null && stack.stackTagCompound != null && stack.stackTagCompound.hasKey("color")){
				 if(stack.stackTagCompound.getString("color").contains("BlockRedstone")) dir |= 8;
				 if(stack.stackTagCompound.getString("color").contains("Lightgem")) dir |= 4;
		 }
	...
	public void registerIcons(IconRegister register) {
		...
		//THIS REGISTERS THE FLUIDS' TEXTURES MUAHAHAHAHA NO NEED FOR FLUID BLOCKS
	...
	public boolean removeBlockByPlayer(World world, EntityPlayer player, int x, int y, int z) {
		...
		if(!world.isRemote){
			if( (player.isSneaking()) ?  backpack.equip(world, player, x, y, z) :
				(player.capabilities.isCreativeMode) ? true : backpack.drop(world, x, y, z)){
