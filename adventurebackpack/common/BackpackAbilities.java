...
public class BackpackAbilities {
	...
	public static boolean hasAbility(String colorName){
		for(String valid : validWearingBackpacks){
			if(valid.equals(colorName)) return true;
		}
		return false;
	}
		...
		if(backpack instanceof ItemStack && player != null){
			for(String valid : validWearingBackpacks){
				if(valid.equals(colorName)){
					try {
						this.getClass().getMethod("item"+valid, EntityPlayer.class, World.class, ItemStack.class).invoke(instance, player, world, (ItemStack)backpack);
					} catch (Exception oops) {
						//oops.printStackTrace(); Discard silently, nobody cares.
					}
				}
			}
			...
			for(String valid : validTileBackpacks){
				if(valid.equals(colorName)){
					try {
						this.getClass().getMethod("tile"+valid, World.class, TileAdvBackpack.class).invoke(instance, world, (TileAdvBackpack)backpack);
					} catch (Exception oops) {
						//oops.printStackTrace(); Discard silently, nobody cares.
					}
				}
	...
	private boolean isUnderRain(EntityPlayer player){
		return player.worldObj.canLightningStrikeAt(MathHelper.floor_double(player.posX), MathHelper.floor_double(player.posY), MathHelper.floor_double(player.posZ)) 
				|| player.worldObj.canLightningStrikeAt(MathHelper.floor_double(player.posX), MathHelper.floor_double(player.posY + (double)player.height), MathHelper.floor_double(player.posZ));
	}
	...
	public void itemCactus(EntityPlayer player, World world, ItemStack backpack){
		if(Utils.isWearing(player)){
			...
			int drops = 0;
			if(player.isInWater()) drops += 2;
			if(isUnderRain(player))drops += 1;
	...
	public void itemPig(EntityPlayer player, World world, ItemStack backpack){
		if(Utils.isWearing(player)){
			int oinkTime = backpack.stackTagCompound.hasKey("lastTime") ? backpack.stackTagCompound.getInteger("lastTime") - 1: toTicks(30);
			if(oinkTime <= 0){
		...
		if(Utils.isWearing(player)){
			if(player.onGround && player.isSprinting()){
				...
				for (int j = 0; j < i * 2; ++j)
	            {
	                ...
	                world.spawnParticle("slime", player.posX + (double)f2, player.boundingBox.minY, player.posZ + (double)f3, 0.0D, 0.0D, 0.0D);
	            }
	...
	public void itemChicken(EntityPlayer player, World world, ItemStack backpack){
		...
		if(Utils.isWearing(player)){
			int eggTime = backpack.stackTagCompound.hasKey("lastTime") ? backpack.stackTagCompound.getInteger("lastTime") - 1: toTicks(300);
			if(eggTime <= 0){
	...
	public void itemMelon(EntityPlayer player, World world, ItemStack backpack){
		if(Utils.isWearing(player)){
			...
			int drops = 0;
			if(player.isInWater()) drops += 5;
			if(isUnderRain(player))drops += 2;
