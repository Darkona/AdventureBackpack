...
public class Utils {
	...
	public static ItemStack getWearingBackpack(EntityPlayer player){
		if(isWearing(player))return player.inventory.armorInventory[2];
		return null;
	}
	
	public static ItemStack getHoldingBackpack(EntityPlayer player){
		if(isHolding(player))return player.inventory.getCurrentItem();
		return null;
	}
	...
	public static ResourceLocation getBackpackColor(TileAdvBackpack tile) {
		ResourceLocation color;
		if(tile.getColor().isEmpty()){tile.setColor("Standard");}
	...
	public static boolean getPossibleDyes(ItemStack stack) {
		...
		for(Block blej : blockies){
			if(stack.getItem().itemID == blej.blockID)return true;
		}
		
		for(Item lol : items){
			if(stack.getItem() == lol)return true;
		}
		return false;
	}
