...
public class Utils
{
	...
	public static boolean isWearing(EntityPlayer player)
	{
		return player.inventory.armorInventory[2] != null
			&& player.inventory.armorInventory[2].getItem() instanceof ItemAdvBackpack;
	}

	public static boolean isHolding(EntityPlayer player)
	{
		return player.inventory.getCurrentItem() != null
			&& player.inventory.getCurrentItem().getItem() instanceof ItemAdvBackpack;
	}
	...
	public static boolean shouldGiveEmpty(ItemStack cont)
	{
		...
		try
		{
			...
			if (ic2.api.item.Items.getItem("cell").getClass()
				.isInstance(cont.getItem()))
			{
				valid = false;
			}
			// Forestry capsules
			if (java.lang.Class.forName(
				"forestry.core.items.ItemLiquidContainer").isInstance(
				cont.getItem()))
			{
				valid = false;
			}
	...
	public static ResourceLocation getBackpackColor(ItemStack item)
	{
		...
		if (!item.stackTagCompound.hasKey("color")
			|| item.stackTagCompound.getString("color").isEmpty())
		{
			item.stackTagCompound.setString("color", "Standard");
		}
		color =
			new ResourceLocation("adventurebackpack",
				"textures/backpack/backpack"
					+ item.stackTagCompound.getString("color") + ".png");
		return color;
	}
	...
	public static ResourceLocation getBackpackColor(TileAdvBackpack tile)
	{
		...
		color =
			new ResourceLocation("adventurebackpack",
				"textures/backpack/backpack" + tile.getColor() + ".png");
		return color;
	}

	public static boolean getPossibleDyes(ItemStack stack)
	{
		if (OreDictionary.getOreName(OreDictionary.getOreID(stack)).contains(
			"dye"))
		{
			return true;
		}
	...
	public static Block blockies[] = {Block.melon, Block.cactus,
		Block.glowStone, Block.blockRedstone, Block.snow, Block.sponge,
		Block.obsidian, Block.sandStone, Block.chest, Block.coalBlock,
		Block.hay, Block.pumpkin, Block.blockNetherQuartz, Block.blockEmerald,
		Block.blockDiamond, Block.blockIron, Block.blockSnow, Block.bookShelf,
		Block.blockGold, Block.cloth, Block.dragonEgg, Block.enchantmentTable,
		Block.mushroomRed, Block.mushroomBrown, Block.blockLapis};

	public static Item[] items = {

	Item.blazeRod, Item.eyeOfEnder, Item.feather, Item.spiderEye,
		Item.slimeBall, Item.egg, Item.cake, Item.leather, Item.skull,
		Item.beefRaw, Item.porkRaw, Item.cookie, Item.enderPearl,
		Item.magmaCream, Item.netherStar, Item.bone, Item.fishRaw,
		Item.ghastTear, Item.bowlSoup, Item.netherStalkSeeds};
	...
	public static ChunkCoordinates findBlock(World world, int x, int y, int z,
		int BlockID, int range)
	{
