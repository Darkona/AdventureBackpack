...
public class Hose extends ItemBucket implements IFluidContainerItem
{
	...
	{
		super(par1, par2);
		setMaxStackSize(1).setFull3D().setCreativeTab(CreativeTabs.tabTools)
	...
	{
		itemIcon =
			register.registerIcon(Textures.TEXTURE_LOCATION + ":"
				+ ItemInfo.HOSE_ICON);
	}
		...
		boolean par5)
	{
		...
		if (backpack != null && backpack.getItem() instanceof ItemAdvBackpack
			&& backpack.stackTagCompound != null)
		{
			if (backpack.stackTagCompound.getCompoundTag(name).hasKey("Amount"))
			{

				amount =
					backpack.stackTagCompound.getCompoundTag(name).getInteger(
						"Amount");
				fluid =
					backpack.stackTagCompound.getCompoundTag(name).getString(
						"FluidName");
			}
		}
		...
		Entity entity)
	{
		...
		if (mode == 0)
		{
			...
			FluidTank tank =
				(name == "leftTank") ? inventory.getLeftTank()
		...
		int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		...
		if (te != null && te instanceof IFluidHandler)
		{
			switch (getHoseMode(stack, backpack))
			{
				...
				case 0 : // Suck mode
					if (tank
						.fill(((IFluidHandler) te).drain(
							ForgeDirection.UNKNOWN, Constants.bucket, false),
							false) >= Constants.bucket)
					{
						tank.fill(((IFluidHandler) te).drain(
							ForgeDirection.UNKNOWN, Constants.bucket, true),
							true);
						inventory.onInventoryChanged();
						return true;
					}
				...
				case 1 :// Spill mode
					...
					{
						if (((IFluidHandler) te).fill(ForgeDirection.UNKNOWN,
							tank.drain(Constants.bucket, false), false) >= Constants.bucket)
						{
							((IFluidHandler) te).fill(ForgeDirection.UNKNOWN,
								tank.drain(Constants.bucket, true), true);
							inventory.onInventoryChanged();
							return true;
						}
					}
	...
	public String getItemDisplayName(ItemStack stack)
	{
		...
		{
			mode =
				(stack.stackTagCompound.hasKey("mode"))
					? stack.stackTagCompound.getInteger("mode") : mode;
		}
		...
		if (tank != null)
		{
			switch (getHoseMode(stack, backpack))
			{
				case 0 : // If it's in Suck Mode
					...
					{
						HoseSuckEvent suckEvent =
							new HoseSuckEvent(player, stack, world, mop, tank);
						...
						&& mop.typeOfHit == EnumMovingObjectType.ENTITY)
					{
						...
						{
							tank.fill(new FluidStack(Fluids.milk,
								Constants.bucket), true);
							inventory.onInventoryChanged();
						}
					}
					break;
				case 1 : // If it's in Spill Mode
						...
						&& mop.typeOfHit == EnumMovingObjectType.TILE)
					{
						...
						HoseSpillEvent spillEvent =
							new HoseSpillEvent(player, world, x, y, z, tank);
	...
	@Override
	public boolean
		onEntitySwing(EntityLivingBase entityLiving, ItemStack stack)
	{
		// TODO Auto-generated method stub
		return false;
	}
	...
	public EnumAction getItemUseAction(ItemStack stack)
	{
		if (stack.stackTagCompound != null
			&& stack.stackTagCompound.hasKey("mode"))
		{
			return (stack.stackTagCompound.getInteger("mode") == 2)
				? EnumAction.drink : EnumAction.none;
		}
	...
	public FluidStack getFluid(ItemStack container)
	{
		if (container.stackTagCompound != null
			&& container.stackTagCompound.hasKey("FluidName"))
		{
			FluidStack fluid =
				new FluidStack(
					FluidRegistry.getFluid(container.stackTagCompound
						.getString("FluidName")),
	...
	public int getCapacity(ItemStack container)
	{
		int cap = 4000;
		if (container.stackTagCompound != null
			&& container.stackTagCompound.hasKey("Amount"))
		{
