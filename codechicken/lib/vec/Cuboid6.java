...
public class Cuboid6
{
	...
	public Cuboid6(double minx, double miny, double minz, double maxx,
	...
	{
		return AxisAlignedBB.getBoundingBox(min.x, min.y, min.z, max.x, max.y,
			max.z);
	}
	...
	{
		block.setBlockBounds((float) min.x, (float) min.y, (float) min.z,
	...
	public boolean intersects(Cuboid6 b)
	{
		return max.x - 1E-5 > b.min.x && b.max.x - 1E-5 > min.x
			&& max.y - 1E-5 > b.min.y && b.max.y - 1E-5 > min.y
	...
	public String toString()
	{
		...
		return "Cuboid: (" + new BigDecimal(min.x, cont) + ", "
			+ new BigDecimal(min.y, cont) + ", " + new BigDecimal(min.z, cont)
